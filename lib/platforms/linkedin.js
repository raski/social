...
/**
 * Hämtar gilla-markeringar och kommentarer för ett LinkedIn-inlägg via Social Actions API.
 * EXPERIMENTELL: LinkedIns API för statistik på personliga profiler är mer begränsat än
 * för Facebook/Mastodon/Bluesky/X, och det här anropet kan misslyckas beroende på kontots
 * behörighetsnivå. Om det ger ett fel betyder det oftast att statistik för det här inlägget
 * inte är tillgängligt via det enkla (icke-partner) API-flödet.
 */
async function getStats(creds, shareUrn) {
	const url = `https://api.linkedin.com/rest/socialActions/${encodeURIComponent(shareUrn)}`;
	...
	return {
		...
		comments: body.commentsSummary?.totalFirstLevelComments ?? 0,
		shares: null,
	};
}
